...
function CurrentRankCard({ loading }: { loading: boolean }) {
  ...
  const rankData = useQuery(
    api.functions.student.dashboard.getStudentRankAndPosition,
    token ? { token } : "skip"
  )
    ...
    <Card>
      ...
      <CardContent>
        ...
        <p className="text-xs text-muted-foreground">
          Out of {rankData.totalStudents} active students
        </p>
      </CardContent>
    </Card>
  )
...
function LeaderboardCard({ loading }: { loading: boolean }) {
  ...
  const leaderboard = useQuery(
    api.functions.student.dashboard.getStudentLeaderboard,
    token ? { token } : "skip"
  )
            ...
            <div key={student.student_id} className="flex flex-col items-center">
              <div className="relative">
                <Avatar className="h-16 w-16">
                  <AvatarImage src={student.profile_image ? `/api/storage/${student.profile_image}` : undefined} />
              ...
              <p className="text-xs text-primary text-center">
                {student.totalPoints} points • {student.tournamentsCount} tournaments
              </p>
            </div>
          ) : (

            <div key={student.student_id} className="flex items-center gap-3 py-1">
              <div className="relative">
              ...
              <div className="text-right">
                <div className="text-sm font-medium">#{student.rank}</div>
              </div>
            </div>
          )
        ))}
...
export default function StudentDashboard() {
  ...
  const dashboardStats = useQuery(
    api.functions.student.dashboard.getStudentDashboardStats,
    token ? { token } : "skip"
  )

  const performanceData = useQuery(
    api.functions.student.dashboard.getStudentPerformanceTrend,
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold">{greeting}, {user?.name?.split(' ')[0] || 'Student'}!</h1>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="flex flex-col custom:flex-row md:items-stretch space-y-3 md:space-y-0 md:space-x-3">
                  <StatCard
                    ...
                    subtitle="Attended and Unattended"
                ...
                <div className="flex flex-col md:flex-row md:items-stretch space-y-3 md:space-y-0 md:space-x-3">
                  ...
                  <StatCard
                    ...
                    value={dashboardStats?.tournamentsAttendedThisYear || 0}
                    subtitle="This Year"
              ...
              {performanceData === undefined ? (
                ...
                <div className="h-[200px] flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Participate in tournaments to see your performance trend
                    </p>
                  </div>
                </div>
              ) : (
                <ChartContainer config={chartConfig} className="aspect-auto h-[200px] w-full">
                  ...
                  >
                    ...
                    <Area
                      ...
                      type="natural"
                      fill="var(--color-student_performance)"
                      fillOpacity={0.4}
                      stroke="var(--color-student_performance)"
                      strokeWidth={2}
                    />
                  </AreaChart>
                </ChartContainer>
