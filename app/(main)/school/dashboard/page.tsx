...
function CurrentRankCard({ loading }: { loading: boolean }) {
  ...
  const rankData = useQuery(
    api.functions.school.dashboard.getSchoolRankAndPosition,
    token ? { token } : "skip"
  )
    ...
    <Card>
      ...
      <CardContent>
        ...
        <p className="text-xs text-muted-foreground">
          Out of {rankData.totalSchools} active schools
        </p>
      </CardContent>
    </Card>
  )
...
function LeaderboardCard({ loading }: { loading: boolean }) {
  ...
  const leaderboard = useQuery(
    api.functions.school.dashboard.getSchoolLeaderboard,
    token ? { token } : "skip"
  )
    ...
    <Card>
      ...
      <CardContent className="space-y-1">
        {leaderboard.map((school, index) => (
          index === 0 ? (
            <div key={school.school_id} className="flex flex-col items-center">
              ...
              <p className="text-xs text-primary text-center">
                {school.totalPoints} points • {school.totalWins} wins
              </p>
            </div>
          ) : (

            <div key={school.school_id} className="flex items-center gap-3 py-1">
              <div className="relative">
              ...
              <div className="text-right">
                <div className="text-sm font-medium">#{school.rank}</div>
              </div>
            </div>
          )
        ))}
...
export default function SchoolDashboard() {
  ...
  const dashboardStats = useQuery(
    api.functions.school.dashboard.getSchoolDashboardStats,
    token ? { token } : "skip"
  )

  const performanceData = useQuery(
    api.functions.school.dashboard.getSchoolPerformanceTrend,
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold">{greeting}, {user?.name?.split(' ')[0] || 'School Admin'}!</h1>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="flex flex-col custom:flex-row md:items-stretch space-y-3 md:space-y-0 md:space-x-3">
                  <StatCard
                    ...
                    value={dashboardStats?.activeDebaters || 0}
                    subtitle="Lifetime"
                    percentage={dashboardStats?.debatersGrowth || 0}
                    loading={isLoading}
                  />
                </div>
                ...
                <div className="flex flex-col md:flex-row md:items-stretch space-y-3 md:space-y-0 md:space-x-3">
                  ...
                  <StatCard
                    ...
                    value={dashboardStats?.tournamentsAttendedLastYear || 0}
                    subtitle="Last Year"
                    percentage={dashboardStats?.tournamentsGrowth || 0}
                    loading={isLoading}
                  />
                </div>
              ...
              {performanceData === undefined ? (
                ...
                <div className="h-[200px] flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Participate in tournaments to see your school&#39;s performance trend
                    </p>
                  </div>
                </div>
              ) : (
                <ChartContainer config={chartConfig} className="aspect-auto h-[200px] w-full">
                  ...
                  >
                    ...
                    />
                    <Area
                      dataKey="school_performance"
                      type="natural"
                      fill="var(--color-school_performance)"
                      fillOpacity={0.4}
                      stroke="var(--color-school_performance)"
                      strokeWidth={2}
                    />
                  </AreaChart>
                </ChartContainer>
